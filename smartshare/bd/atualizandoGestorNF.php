...
$resultadoCount = oci_parse($conn, $queryCount);
...
while(($count = oci_fetch_array($resultadoCount, OCI_BOTH)) != false)  {
    ...
    if($count['APROVADOR_FILIAL'] == $idGestor){
        ...
        echo $update;
        echo '<br />';
        $resultado = oci_parse($conn, $update);
    ...
    if($count['APROVADOR_AREA'] == $idGestor){
        ...
        echo '<br />';
        $resultado_APROVADOR_AREA = oci_parse($conn, $update_APROVADOR_AREA);
    ...
    if($count['APROVADOR_MARCA'] == $idGestor){
        ...
        echo '<br />';
        $resultado_APROVADOR_MARCA = oci_parse($conn, $update_APROVADOR_MARCA);
    ...
    if($count['APROVADOR_SUPERINTENDENTE'] == $idGestor){
        ...
        echo '<br />';
        $resultado_APROVADOR_SUPERINTENDENTE = oci_parse($conn, $update_APROVADOR_SUPERINTENDENTE);
    ...
    if($count['APROVADOR_GERENTE'] == $idGestor){
        ...
        echo '<br />';
        $resultado_APROVADOR_GERENTE = oci_parse($conn, $update_APROVADOR_GERENTE);
    ...
    if($count['APROVADOR_GESTOR'] == $idGestor){
        ...
        echo '<br />';
        $resultado_APROVADOR_GESTOR = oci_parse($conn, $update_APROVADOR_GESTOR);
        oci_execute($resultado_APROVADOR_GESTOR, OCI_COMMIT_ON_SUCCESS);
    }

}

header('location: ../front/smartshare_pag.php?id_drop=12&id_usuario='.$_GET['id_usuario'].'&buscar=1&msn=1');
