...
if(($rowDep = oci_fetch_array($resultDep, OCI_BOTH)) != FALSE) { 
    ...
    if (($rowDep['EMPRESA_DEPARTAMENTO'] != NULL) || ($rowDep['APROVADORESRH'] != NULL) || ($rowDep['GESTORDIRETO'] != NULL)) {
        header('location: ../front/smartshare_pag.php?id_drop=2&msn=5&id_usuario='. $_GET['id_usuario'] .'');//msn=5 Departamento já possui relação em outra tabela
    }else{
    ...
    header('location: ../front/smartshare_pag.php?id_drop=2&msn=3&id_usuario='. $_GET['id_usuario'] .'');//msn=3 deletado com sucesso
