...
if (($rowverif = oci_fetch_array($resultVerif, OCI_BOTH)) != FALSE) {
    ...
    header('location: ../front/smartshare_pag.php?id_drop=4&msn=4&id_usuario='. $_GET['id_usuario'] .''); //Já existe uma empresa com esse departamento cadastrado!
    ...
    if (oci_execute($resultInsert)) {
        header('location: ../front/smartshare_pag.php?id_drop=4&msn=1&id_usuario='. $_GET['id_usuario'] .''); //msn 1 Salvo com sucesso!
    }else{
