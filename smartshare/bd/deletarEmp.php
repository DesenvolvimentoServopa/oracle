...
$resultEmp = ociparse($conn, $selectEmp);
...
if(($rowEmp = oci_fetch_array($resultEmp, OCI_BOTH)) != FALSE) { 
    ...
    if (($rowEmp['TABELA_EMPRESADEPARTAMENTO'] != NULL) || ($rowEmp['TABELA_APROVADORESRH'] != NULL) || ($rowEmp['TABELA_GESTORDIRETO'] != NULL)) {
        header('location: http://10.100.1.214/unico/sistemas/sisrev/front/empresas.php?pg=2&tela=2&msn=14');//msn=5 Empresa já possui relacionamento
    }else{       
    ...
    $resultUpdateDeleteEmp = oci_parse($conn, $deleteEmpresa);
    oci_execute($resultUpdateDeleteEmp);
        
    header('location: http://10.100.1.214/unico/sistemas/sisrev/front/empresas.php?pg=2&tela=2&msn=14');//msn=3 Empresa deletada com sucesso
